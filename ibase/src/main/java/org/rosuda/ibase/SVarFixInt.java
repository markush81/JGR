...
public class SVarFixInt extends SVar {
    ...
    public double atD(int i) {
        return (i < 0 || i >= insertPos || cont[i] == int_NA) ? double_NA : cont[i];
    }

    public int atI(int i) {
        return (i < 0 || i >= insertPos) ? int_NA : ((int) (cont[i] + 0.5));
    }

    public String asS(int i) {
        return (i < 0 || i >= insertPos || cont[i] == int_NA) ? null : Integer.toString(cont[i]);
    }
